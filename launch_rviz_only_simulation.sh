...
# RViz-Only Stretch Robot Simulation Launcher
# Starts: Headless MuJoCo + SLAM + RViz + Web Controller
# Maximum performance with no MuJoCo GUI - visualization only in RViz
...
cleanup_all() {
    ...
    pkill -f "stretch_rviz_only_bridge" 2>/dev/null || true
    pkill -f "simple_rviz_web_controller" 2>/dev/null || true
...
print_header "🖥️ STRETCH ROBOT RVIZ-ONLY SIMULATION"
print_color "Components: Headless MuJoCo + SLAM + RViz + Web Controller" $CYAN
print_color "🚀 MAXIMUM PERFORMANCE: No MuJoCo GUI - RViz visualization only" $YELLOW

# Optional: Check if optimized assets exist (not required for headless mode)
if [ -d "$HOME/Downloads/assets" ]; then
    ASSET_COUNT=$(ls ~/Downloads/assets/*.obj ~/Downloads/assets/*.stl 2>/dev/null | wc -l)
    print_color "📁 Found $ASSET_COUNT optimized mesh files (optional for headless mode)" $CYAN
else
    print_color "📁 No optimized assets found - using default meshes (headless mode)" $CYAN
fi
...
print_header "🚀 STARTING RVIZ-ONLY SIMULATION COMPONENTS"

# Step 1: Start RViz-Only MuJoCo SLAM Bridge (forced headless)
print_color "🔧 Step 1: Starting headless MuJoCo simulation with SLAM bridge..." $GREEN
...
# Step 2: Start Robot State Publisher with proper Stretch URDF
print_color "🔧 Step 2: Starting robot state publisher with Stretch URDF..." $GREEN

# Copy URDF to temporary location and use parameter file approach
URDF_FILE="/tmp/stretch_rviz_robot.urdf"
cp "/home/user/ament_ws/install/stretch_description/share/stretch_description/urdf/stretch.urdf" "$URDF_FILE"

ros2 run robot_state_publisher robot_state_publisher \
    --ros-args -p use_sim_time:=true \
    --param-file <(echo "robot_state_publisher:
  ros__parameters:
...
print_header "🎉 RVIZ-ONLY SIMULATION READY!"

print_color "" $NC
print_color "📺 WINDOWS YOU SHOULD SEE:" $PURPLE
print_color "  📊 RViz: Robot model, laser scans, and real-time map building" $CYAN
print_color "  🚫 MuJoCo GUI: DISABLED for maximum performance" $CYAN
if [ "$NO_WEB" != "true" ]; then
    print_color "  🌐 Web Browser: http://localhost:$WEB_PORT (robot control interface)" $CYAN
fi

print_color "" $NC
print_color "🚀 RVIZ-ONLY PERFORMANCE DETAILS:" $PURPLE
print_color "  • MuJoCo runs in headless mode (no GUI rendering)" $CYAN
print_color "  • All visualization through RViz only" $CYAN
print_color "  • Expected: 5-20x better performance vs GUI mode" $CYAN
print_color "  • Cameras disabled by default for maximum speed" $CYAN
...
print_color "⏳ Monitoring RViz-only simulation... Press Ctrl+C to stop" $GREEN
